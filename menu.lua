...
function TabClass:AddLeftTabbox()
	return self:AddLeftGroupbox("Tabbox")
end

function TabClass:AddRightTabbox()
	return self:AddRightGroupbox("Tabbox")
end

function GroupboxClass:AddTab(name)
    return self
end
...
local function CreateChainingWrapper(parent, element, idx, type)
    ...
    function wrapper:AddColorPicker(c_idx, c_options)
        return parent:AddColorPicker(c_idx, c_options)
    end
    function wrapper:AddKeyPicker(k_idx, k_options)
        return parent:AddKeyPicker(k_idx, k_options)
    end
    
    function wrapper:OnChanged(callback)
        if type == "toggle" then
            create_connection(element.on_toggle_change, function(val)
                wrapper.Value = val
                callback(val)
            end)
        elseif type == "slider" then
            create_connection(element.on_slider_change, function(val)
                wrapper.Value = val
                callback(val)
            end)
        elseif type == "dropdown" then
            create_connection(element.on_dropdown_change, function(val)
                wrapper.Value = val
                callback(val)
            end)
        elseif type == "textbox" then
            create_connection(element.on_textbox_change, function(val)
                wrapper.Value = val
                callback(val)
            end)
        elseif type == "colorpicker" then
            create_connection(element.on_color_change, function(val)
                wrapper.Value = val
                callback(val)
            end)
        elseif type == "keybind" then
            create_connection(element.on_key_change, function(val)
                wrapper.Value = val
                callback(val)
            end)
        end
    ...
    if type == "colorpicker" then
        function wrapper:SetValueRGB(color)
            element:set_colorpicker(color)
        end
    end
...
function GroupboxClass:AddToggle(idx, options)
	local el = self.section:create_element({ name = options.Text }, {
		toggle = { flag = idx, default = options.Default }
	})
	local wrapper = CreateChainingWrapper(self, el, idx, "toggle")
    if options.Callback then wrapper:OnChanged(options.Callback) end
	getgenv().Toggles[idx] = wrapper
	return wrapper
end

function GroupboxClass:AddButton(options)
    local btnOptions = options
...
function GroupboxClass:AddLabel(text)
    local t = text
    if type(t) == "table" then t = t.Text end
	local el = self.section:create_element({ name = t }, {
		info = {}
	})
	local wrapper = { element = el, parent = self }
    function wrapper:AddColorPicker(c_idx, c_options)
...
function GroupboxClass:AddDivider()
    self:AddLabel(" ")
end
...
function GroupboxClass:AddSlider(idx, options)
    ...
    if options.Callback then wrapper:OnChanged(options.Callback) end
	getgenv().Options[idx] = wrapper
	return wrapper
end

function GroupboxClass:AddInput(idx, options)
	local el = self.section:create_element({ name = options.Text or options.Default or "Input" }, {
		textbox = { flag = idx, default = options.Default }
	})
	local wrapper = CreateChainingWrapper(self, el, idx, "textbox")
    if options.Callback then wrapper:OnChanged(options.Callback) end
	getgenv().Options[idx] = wrapper
	return wrapper
end

function GroupboxClass:AddDropdown(idx, options)
	local el = self.section:create_element({ name = options.Text }, {
		dropdown = { flag = idx, options = options.Values, default = {options.Values[options.Default] or options.Default}, requires_one = not options.Multi }
	})
	local wrapper = CreateChainingWrapper(self, el, idx, "dropdown")
    if options.Callback then wrapper:OnChanged(options.Callback) end
	getgenv().Options[idx] = wrapper
	return wrapper
end
...
function Library:Unload()
    local env = getgenv()
    if env["_JUJU"] then env["_JUJU"]() end
end

function Library:OnUnload(callback)
    local old_unload = getgenv()["_JUJU"]
    getgenv()["_JUJU"] = function()
        callback()
        if old_unload then old_unload() end
    end
end
...
function ThemeManager:ApplyToGroupbox(...) end
